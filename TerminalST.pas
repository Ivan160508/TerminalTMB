...
uses
  ...
  Dialogs, StdCtrls, ComCtrls, ExtCtrls, Registry, Clipbrd, DateUtils, ShellApi,
    Math, mmsystem, Common;

type
  TTypeFilterLog = (TFLCapt = 1, TFLOut = 2, TFLNoFilter = 3, TFLNoOut = 4);
...
type
  ...
  TForm1 = class(TForm)
    ...
    function SendPortData(Data: string; AddData: string; SendMode:
      TTypeSendMode; isSaveSend: boolean): boolean;
    function ConvAddStr(Data: string; Tail: string; SendMode: TTypeSendMode):
      string;
    ...
    function CalcCsum(Cmd: string; NumCsum: integer; SendMode: TTypeSendMode):
      integer;
    ...
    procedure MemoUpd(DT: TDateTime; BeginStr: string; LogStr: string;
      LogStrASCII: string; LogStrHEX: string; LogStrDec: string; LogStrCUST:
      string; isRx: boolean; ColorText: TColor);
    procedure PauseLog;
  ...
  public
    ...
    procedure SetActBtSendInAdd(nPort: Integer; isActiv: Boolean; Name: string;
      Hint: string);
...
var
  ...
  iReadBuf: Cardinal;
  iWriteBuf: Cardinal;
  CntDataReadBufAv: Cardinal;
    //число доступных байтов для чтения из кольцевого буфера;
  ...
  RXRead: Cardinal;
    //Используется для подсчёта процента заполнения буфера(число прочитанных байтов)
  RXOut: Cardinal;
    //Используется для подсчёта процента заполнения буфера(число отображённых байтов)
...
begin
  if EventLog.Lines.Count > 1000 then
    EventLog.Clear;
...
procedure TForm1.MemoUpd(DT: TDateTime; BeginStr: string; LogStr: string;
  LogStrASCII: string; LogStrHEX: string; LogStrDec: string; LogStrCUST: string;
  isRx: boolean; ColorText: TColor);
var
  lst: TStringList;
  isOutLog: boolean;
  isPauseLog: boolean;
...
begin
  ...
  if (isSkippingReps and (LogStr = RxDataLogOld)) then
    Exit;
  RxDataLogOld := LogStr;
  ///////////////////////////////////////////////////////////////////////////////
  ...
  if (isAutoClrStrASCII) and (AutoClrStrASCII <> '') and (isRx) and (ReadMode =
    ReadASCII) then
  begin
  ...
  if (isAutoClrStrHEX) and (AutoClrStrHEX <> '') and (isRx) and (ReadMode =
    ReadHEX) then
  begin
  ...
  begin
    ...
    CntCmpLvl.Caption := 'Match cnt ' + '"' + FiltrLog.Text + '" : ' +
      IntToStr(Stats.HitCounter);
  end;
  ...
  begin
    ...
    begin
      if isShowMode and (StrASCII <> '') then
        StrASCII := '[A] ' + StrASCII;
      if isShowMode and (StrHEX <> '') then
        StrHEX := '[H] ' + StrHEX;
      if isShowMode and (StrDEC <> '') then
        StrDEC := '[D] ' + StrDEC;
      if isShowMode and (StrCUST <> '') then
        StrCUST := '[C] ' + StrCUST;
    end;
  ...
  if ((isADDRX and isRx) or (isADDTX and not isRx)) and (isAddIgnFlt or isOutLog)
    then
  begin
    CMainWindowAdd.SelAttributes.Color := ColorText;
    if StrASCII <> '' then
...
begin
  ...
  begin
    ...
    AssignFile(ConfigFile2, ExtractFilePath(Application.ExeName) +
      'ListConfig');
...
begin
  ...
  if Length(Form1.ComPort.Text) = 4 then
    CurrentComPort := PChar(Form1.ComPort.Text);
      //(Form1.ComPort.Items[Form1.ComPort.ItemIndex]);
  if Length(Form1.ComPort.Text) > 4 then
    CurrentComPort := PChar('\\.\' + Form1.ComPort.Text);
      //(Form1.ComPort.Items[Form1.ComPort.ItemIndex]);
  ...
  hCom := CreateFile(CurrentComPort, Generic_Read or Generic_Write, 0, nil,
...
begin
  ...
  begin
    ...
    PurgeComm(hCom, PURGE_TXABORT or PURGE_RXABORT or PURGE_TXCLEAR or
      PURGE_RXCLEAR);
    CloseHandle(hCom);
    isOpenComPort := false;
...
begin
  ...
  begin
    ...
    OldCmdList.Width := MainStrCmd.Width;
      // AddStrCmd.Left + AddStrCmd.Width;//  MainWindow.Width;
    ...
    StaticText1.Width := STLog.Width;
      // Button41.Left + Button41.Width - OpenLogBT.Width - 2;
...
var
  ...
  isCmdCfg_2: Boolean;
begin
  VersionInfo := 'TerminalTMB v7.151b';
  DateInfo := '23.03.2026';
  ...
  isCmdCfg_2 := isCmdCfg_2 and
    (Get_File_Size(ExtractFilePath(Application.ExeName) + 'ListConfig', True) >
    0);
  ...
  isCmdCfg_1 := isCmdCfg_1 and
    (Get_File_Size(ExtractFilePath(Application.ExeName) + 'CmdConfig', True) > 0);
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          ListsCmd[NumList].Options[2] := ListsCmd[NumList].Options[2] or
            $8000000;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          for j := 1 to 12 do
            ListsCmd2[NumList].NameHelp[i, j] := ListsCmd[NumList].NameHelp[i,
              j];
        end;
        ListsCmd2[NumList].Marker := MARKER;
        ...
        if (ListsCmd[NumList].Options[2] and $1) = $1 then
          ListsCmd2[NumList].SendMode := SendHEX
        else
          ListsCmd2[NumList].SendMode := SendASCII;
        if (ListsCmd[NumList].Options[2] and $2) = $2 then
          ListsCmd2[NumList].ReadMode := ReadHEX
        else
          ListsCmd2[NumList].ReadMode := ReadASCII;
      end;
    end;
    RenameFile(ExtractFilePath(Application.ExeName) + 'CmdConfig',
      ExtractFilePath(Application.ExeName) + 'CmdConfigOld');
  ...
  if not FileExists(ExtractFilePath(Application.ExeName) + 'OldCmdList') or
    (Get_File_Size(ExtractFilePath(Application.ExeName) + 'OldCmdList', True) = 0)
    then
  begin
    ReWrite(OldCmdFile);
  ...
  if not FileExists(ExtractFilePath(Application.ExeName) + 'CnfgTMB') or
    (Get_File_Size(ExtractFilePath(Application.ExeName) + 'CnfgTMB', True) = 0)
    then
  begin
  ...
  begin
    ...
    if (CfgTerminal.TimeNoInputData = 0) or (CfgTerminal.TimeNoInputData > 60)
      then
      TimeNoInputData := 20
    else
  ...
  if isAddASCII then
    CfgTerminal.AddWinBits := CfgTerminal.AddWinBits or $01;
  if isAddHEX then
    CfgTerminal.AddWinBits := CfgTerminal.AddWinBits or $02;
  if isAddTX then
    CfgTerminal.AddWinBits := CfgTerminal.AddWinBits or $04;
  if isAddRX then
    CfgTerminal.AddWinBits := CfgTerminal.AddWinBits or $08;
  if isAddIgnFlt then
    CfgTerminal.AddWinBits := CfgTerminal.AddWinBits or $10;
  if isShowMode then
    CfgTerminal.AddWinBits := CfgTerminal.AddWinBits or $40;
  if isAddDec then
    CfgTerminal.AddWinBits := CfgTerminal.AddWinBits or $80;
  if isAddCUST then
    CfgTerminal.AddWinBits := CfgTerminal.AddWinBits or $100;
  /////////////////////////////////////////////////////////////////////////
  /////////////////////////////////////////////////////////////////////////
...
{$IOCHECKS off}
    ...
    begin
      isWriteEvent := false;
      ShowMessage('Error accessing the log file "Events.txt": ' +
        IntToStr(errIO));
...
{$IOCHECKS on}
    if CfgTerminal.NameLogEvents = '' then
      WriteLogEvent(Now, 'The log file was created in the program ' +
        VersionInfo);
  ...
  begin
    ...
    MainPanel.Visible := false;
    CMainWindow.Height := CMainWindow.Height + LogPanel.Height +
      MainPanel.Height;
    SecondWindow.Height := SecondWindow.Height + LogPanel.Height +
      MainPanel.Height;
    CMainWindowAdd.Height := CMainWindowAdd.Height + LogPanel.Height +
      MainPanel.Height;
    SecondWindowAdd.Height := SecondWindowAdd.Height + LogPanel.Height +
      MainPanel.Height;
...
function TForm1.CalcCsum(Cmd: string; NumCsum: integer; SendMode:
  TTypeSendMode): integer;
var
  SumIndex: integer;
  Summa: integer;
  TmpStr: string;
  ...
  begin
    ...
    begin
      ...
      if (tmp and $01) <> 0 then
        tmpsum := tmpsum xor $5E;
      if (tmp and $02) <> 0 then
        tmpsum := tmpsum xor $BC;
      if (tmp and $04) <> 0 then
        tmpsum := tmpsum xor $61;
      if (tmp and $08) <> 0 then
        tmpsum := tmpsum xor $C2;
      if (tmp and $10) <> 0 then
        tmpsum := tmpsum xor $9D;
      if (tmp and $20) <> 0 then
        tmpsum := tmpsum xor $23;
      if (tmp and $40) <> 0 then
        tmpsum := tmpsum xor $46;
      if (tmp and $80) <> 0 then
        tmpsum := tmpsum xor $8C;
    end;
    Summa := tmpsum;
  end;
  result := Summa;
...
function TForm1.ConvAddStr(Data: string; Tail: string; SendMode: TTypeSendMode):
  string;
var
  i: integer;
  posit: integer;
    ...
    begin
      repeat
        ...
        begin
          ...
          if i = 10 then //MODBUS
          begin
            TmpStr := IntToHex((Sum and $FF), 2) + ' ' + IntToHex(((Sum shr 8)
              and $FF), 2);
...
procedure SendAddPortData(Data: string; AddData: string; SendMode:
  TTypeSendMode; port: Integer);
var
  iStrData: Integer;
  iTxData: Dword;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        SymbolCod := HexToInt(Data[iStrData + 1]) * 16 + HexToInt(Data[iStrData
          + 2]);
  ...
  begin
    while (iStrData < Lendata + 1) and (iTxData < 65000) do
    begin
      TxDataSend[iTxData] := HexToInt(Data[iStrData]) * 16 +
        HexToInt(Data[iStrData + 1]);
      TxDataLogHEX := TxDataLogHEX + IntToHex(TxDataSend[iTxData], 2) + ' ';
      TxDataLogASCII := TxDataLogASCII + ChrZ(TxDataSend[iTxData]);
      TxDataLogDEC := TxDataLogDEC + IntToStr(TxDataSend[iTxData]) + ' ';
      TxDataLogCUST := TxDataLogCUST + ChrCustom(TxDataSend[iTxData]);
  ...
  begin
    ...
    DateTimeStr := SetTimeStamp(ListsCmd2[NumListCmd].isVisTime,
      TerminalST.Form1.isAddDate, TimeSend);
    ...
    Form1.MemoUpd(Now, BeginStr, TxDataLog, TxDataLogASCII, TxDataLogHEX,
      TxDataLogDEC, TxDataLogCUST, false, FontColorSend);

    FormPacketsCNT.CheckSubString(TTypeSrcData(port), BeginStr, TxDataLog,
      DirTx);
    ...
    case port of
      1: FormAddPorts.SetSendFromMainForm(@TxDataSend[0], 0, TimeSend, iTxData,
        StrInAddLog, ColorInAddLog);
      2: FormAddPorts.SetSendFromMainForm(@TxDataSend[0], 1, TimeSend, iTxData,
        StrInAddLog, ColorInAddLog);
      3: FormMultiPort.SetSendFromMainForm(@TxDataSend[0], 1, TimeSend, iTxData,
        StrInAddLog, ColorInAddLog);
      4: FormMultiPort.SetSendFromMainForm(@TxDataSend[0], 2, TimeSend, iTxData,
        StrInAddLog, ColorInAddLog);
      5: FormMultiPort.SetSendFromMainForm(@TxDataSend[0], 3, TimeSend, iTxData,
        StrInAddLog, ColorInAddLog);
      6: FormMultiPort.SetSendFromMainForm(@TxDataSend[0], 4, TimeSend, iTxData,
        StrInAddLog, ColorInAddLog);
      7: FormMultiPort.SetSendFromMainForm(@TxDataSend[0], 5, TimeSend, iTxData,
        StrInAddLog, ColorInAddLog);
      8: FormMultiPort.SetSendFromMainForm(@TxDataSend[0], 6, TimeSend, iTxData,
        StrInAddLog, ColorInAddLog);
      9: FormMultiPort.SetSendFromMainForm(@TxDataSend[0], 7, TimeSend, iTxData,
        StrInAddLog, ColorInAddLog);
      10: FormMultiPort.SetSendFromMainForm(@TxDataSend[0], 8, TimeSend,
        iTxData, StrInAddLog, ColorInAddLog);
      11: FormMultiPort.SetSendFromMainForm(@TxDataSend[0], 9, TimeSend,
        iTxData, StrInAddLog, ColorInAddLog);
      12: FormMultiPort.SetSendFromMainForm(@TxDataSend[0], 10, TimeSend,
        iTxData, StrInAddLog, ColorInAddLog);
      13: FormMultiPort.SetSendFromMainForm(@TxDataSend[0], 11, TimeSend,
        iTxData, StrInAddLog, ColorInAddLog);
      14: FormMultiPort.SetSendFromMainForm(@TxDataSend[0], 12, TimeSend,
        iTxData, StrInAddLog, ColorInAddLog);
      15: FormMultiPort.SetSendFromMainForm(@TxDataSend[0], 13, TimeSend,
        iTxData, StrInAddLog, ColorInAddLog);
      16: FormMultiPort.SetSendFromMainForm(@TxDataSend[0], 14, TimeSend,
        iTxData, StrInAddLog, ColorInAddLog);
      17: FormMultiPort.SetSendFromMainForm(@TxDataSend[0], 15, TimeSend,
        iTxData, StrInAddLog, ColorInAddLog);
      18: FormMultiPort.SetSendFromMainForm(@TxDataSend[0], 16, TimeSend,
        iTxData, StrInAddLog, ColorInAddLog);
    end;
...
begin
  ...
  try
    ...
    DateTimeStr := SetTimeStamp(ListsCmd2[NumListCmd].isVisTime,
      TerminalST.Form1.isAddDate, Now);
    ...
    Form1.MemoUpd(Now, PortStr + DateTimeStr + DirStr, TxDataLog, '', '', '',
      '', false, FontColorSend);
    ...
    until (Eof(FileData)) or (cntRead < BUF_TMP_SIZE) or (i =
      MAX_SIZE_FILE_DATA);
    ...
    begin
      ...
      Form1.MemoUpd(Now, PortStr + DateTimeStr + CntBytesStr + DirStr, TxDataLog,
        '', '', '', '', false, FontColorSend);
    end;
    ...
    Form1.MemoUpd(Now, DateTimeStr + DirStr, TxDataLog, '', '', '', '', false,
      FontColorSend);
  end;
  Result := True;
end;

function TForm1.SendPortData(Data: string; AddData: string; SendMode:
  TTypeSendMode; isSaveSend: boolean): boolean;
var
  iStrData: Integer;
  iTxData: Dword;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if (Length(Data) < 255) and (isAddListCmd = false) and (not isSendFile)
          and (isSaveSend) and (not isByteByByte) then
        begin
        ...
        begin
          ...
          TxDataLog := Data;
          Form1.MemoUpd(Now, DateTimeStr + DirStr, TxDataLog, TxDataLog,
            TxDataLog, TxDataLog, TxDataLog, false, FontColorSend);
          Delete(Data, 1, 11);
          SendFile(Data);
        end
        else
        begin
          ...
          begin
            ...
            begin
              if (Data[iStrData] = '$') and (iStrData > LenMain) then
              begin
                SymbolCod := HexToInt(Data[iStrData + 1]) * 16 +
                  HexToInt(Data[iStrData + 2]);
                TxDataSend[iTxData] := SymbolCod;
                if isInvert then
          ...
          begin
            ...
            MemoUpd(Now, BeginStr, TxDataLog, TxDataLogASCII, TxDataLogHEX,
              TxDataLogDEC, TxDataLogCUST, false, FontColorSend);
            FormPacketsCNT.CheckSubString(SrcPortMain, BeginStr, TxDataLog,
              DirTx);
            if isListSendCmd then
            begin
              WriteLogSendData(TxDataLog, SendMode);
...
procedure TForm1.ButtonCmdHelp(Sender: TObject; Shift: TShiftState; X, Y:
  Integer);
var
  numButton: integer;
  i: integer;
...
procedure TForm1.EditCmdHelp(Sender: TObject; Shift: TShiftState; X, Y:
  Integer);
var
  numButton: integer;
  i: integer;
...
begin
  ...
  begin
    ...
    EDTJKey[i].Text := IntToStr(SettJ[i].numJ) + ':' + IntToStr(SettJ[i].Key1) +
      ',' + IntToStr(SettJ[i].Key2) + ' ' + IntToStr(SettJ[i].SendMode);
    ...
    BTCmd[i].Enabled := (not (ListsCmd2[NumList].CmdData[i] = '')) and (not
      isModeEditHK);
    ...
    if HotKey[i] <> 0 then
    begin
      BTCmd[i].Hint := BTCmd[i].Hint + ' ' + '   [ Hot key:' + ' ' +
        Chr(HotKey[i]) + ' ' + ']';
      EDTCmd[i].Hint := EDTCmd[i].Hint + ' ' + '   [ Hot key:' + ' ' +
        Chr(HotKey[i]) + ' ' + ']';
    end;
  end;
...
begin
  ...
  if RBName.Checked = true then
    ListsCmd2[NumListCmd].CmdName[ActualCellListCmd] :=
      EDTCmd[ActualCellListCmd].Text;
  if RbData.Checked = true then
    ListsCmd2[NumListCmd].CmdData[ActualCellListCmd] :=
      EDTCmd[ActualCellListCmd].Text;
  isReqSaveCmdList := true;
end;
...
begin
  ...
  begin
    if (Length(OldCommand[NumCmd].OldCmd) > 0) then
    begin
      if (Pos(MainStrCmd.Text, OldCommand[NumCmd].OldCmd) = 1) or
        (MainStrCmd.Text = '') then
      begin
        //OldCmdList.Items.Add(OldCommand[NumCmd].OldCmd);
        lstFastOldCmd.Items.Add(OldCommand[NumCmd].OldCmd);
      end;
...
begin
  ...
  if (AutoConnect.Checked = false) and (not isStateCom) and (isOpenComPort =
    true) then
    ComPort.Color := clRed;

  if (BTConnect.Tag = 1) and (isOpenComPort = false) and (AutoConnect.Checked =
    false) then
    ComPort.Color := clRed;
...
begin
  ...
  if isSep0D then
    CfgTerminal.LineSeparators := CfgTerminal.LineSeparators or $00001;
  if isSep0A then
    CfgTerminal.LineSeparators := CfgTerminal.LineSeparators or $00002;
  if isSep00 then
    CfgTerminal.LineSeparators := CfgTerminal.LineSeparators or $00004;
  if isSep0D0A then
    CfgTerminal.LineSeparators := CfgTerminal.LineSeparators or $00008;
  if isSepTime then
    CfgTerminal.LineSeparators := CfgTerminal.LineSeparators or $00010;
  if isSepBytes then
    CfgTerminal.LineSeparators := CfgTerminal.LineSeparators or $00040;
  if isSepSymbol then
    CfgTerminal.LineSeparators := CfgTerminal.LineSeparators or $00080;
  if isResetFifo then
    CfgTerminal.LineSeparators := CfgTerminal.LineSeparators or $00100;
  if isAutoExpandNoInData then
    CfgTerminal.LineSeparators := CfgTerminal.LineSeparators or $00200;
  if isAutoExpStr then
    CfgTerminal.LineSeparators := CfgTerminal.LineSeparators or $00400;
  if isCntExpBytes then
    CfgTerminal.LineSeparators := CfgTerminal.LineSeparators or $00800;
  if isExpWin then
    CfgTerminal.LineSeparators := CfgTerminal.LineSeparators or $01000;
  if isFileSendOK then
    CfgTerminal.LineSeparators := CfgTerminal.LineSeparators or $02000;
  if isAutoClrStrHEX then
    CfgTerminal.LineSeparators := CfgTerminal.LineSeparators or $04000;
  if isAutoClrStrASCII then
    CfgTerminal.LineSeparators := CfgTerminal.LineSeparators or $08000;
  if isWriteEvent then
    CfgTerminal.LineSeparators := CfgTerminal.LineSeparators or $10000;
  if isAConnectPort then
    CfgTerminal.LineSeparators := CfgTerminal.LineSeparators or $20000;
  if isSepBefSymbol1 then
    CfgTerminal.LineSeparators := CfgTerminal.LineSeparators or $40000;
  if isSepBefSymbol2 then
    CfgTerminal.LineSeparators := CfgTerminal.LineSeparators or $080000;
  if isSepSymbol2 then
    CfgTerminal.LineSeparators := CfgTerminal.LineSeparators or $100000;
  ...
  if isAutoLog then
    CfgTerminal.FlagsAutoLog := (CfgTerminal.FlagsAutoLog or $00000001);
  if isAutoLogText then
    CfgTerminal.FlagsAutoLog := (CfgTerminal.FlagsAutoLog or $00000002);
  if isAutoLogBin then
    CfgTerminal.FlagsAutoLog := (CfgTerminal.FlagsAutoLog or $00000004);
  if isAutoLogTextTx then
    CfgTerminal.FlagsAutoLog := (CfgTerminal.FlagsAutoLog or $00000008);
  if isAutoLogTextRx then
    CfgTerminal.FlagsAutoLog := (CfgTerminal.FlagsAutoLog or $00000010);
  if isAutoLogBinTx then
    CfgTerminal.FlagsAutoLog := (CfgTerminal.FlagsAutoLog or $00000020);
  if isAutoLogBinRx then
    CfgTerminal.FlagsAutoLog := (CfgTerminal.FlagsAutoLog or $00000040);
  if isAutoLogSaveDelay then
    CfgTerminal.FlagsAutoLog := (CfgTerminal.FlagsAutoLog or $00000080);
...
begin
  ...
  begin
    ...
    begin
      ...
      STLog.Caption := 'Log is written to file ' + NameLogFile + ' ' +
        NameLogBinFile;
  ...
  begin
    repeat
      ...
      if (not IsHex) then
        isTail := ((RxBufData[i - 1] = $0A) and isSep0A) or
          ((RxBufData[i - 1] = $00) and isSep00) or
          ((i > 1) and (RxBufData[i - 2] = $0D) and (RxBufData[i - 1] = $0A) and
            isSep0D0A) or
          ((RxBufData[i - 1] = $0D) and (BufDataRB[iReadBuf] <> $0A) and
            isSep0D);

      isTail := isTail or (isSepBytes and (GlBufRx.Shift + i >= SepBytes)) or (i
        > MAX_SHIFT_BUF_RX);

      isTail := isTail or (isSepAftSeq1_4 and (i > 3) and (SepAftByte1_4_1 =
        RxBufData[i - 4])
        and (SepAftByte1_4_2 = RxBufData[i - 3])
        and (SepAftByte1_4_3 = RxBufData[i - 2])
        and (SepAftByte1_4_4 = RxBufData[i - 1]));

      isTail := isTail or (isSepAftSeq1_3 and (i > 2) and (SepAftByte1_3_1 =
        RxBufData[i - 3])
        and (SepAftByte1_3_2 = RxBufData[i - 2])
        and (SepAftByte1_3_3 = RxBufData[i - 1]));

      isTail := isTail or (isSepAftSeq1_2 and (i > 1) and (SepAftByte1_2_1 =
        RxBufData[i - 2])
      ...
      isTail := isTail or (isSepAftSeq2_4 and (i > 3) and (SepAftByte2_4_1 =
        RxBufData[i - 4])
        and (SepAftByte2_4_2 = RxBufData[i - 3])
        and (SepAftByte2_4_3 = RxBufData[i - 2])
        and (SepAftByte2_4_4 = RxBufData[i - 1]));

      isTail := isTail or (isSepAftSeq2_3 and (i > 2) and (SepAftByte2_3_1 =
        RxBufData[i - 3])
        and (SepAftByte2_3_2 = RxBufData[i - 2])
        and (SepAftByte2_3_3 = RxBufData[i - 1]));

      isTail := isTail or (isSepAftSeq2_2 and (i > 1) and (SepAftByte2_2_1 =
        RxBufData[i - 2])
      ...
      if (isSepBefSymbol1 and (BufDataRB[iReadBufTmp] = SepBefSymbol1)) or
        ...
        ((CntDataReadBufAv = 0) and ((isSepBefSymbol1 and (RxBufData[0] =
          SepBefSymbol1)) or (isSepBefSymbol2 and (RxBufData[0] = SepBefSymbol2))))
          then
      begin
        isTail := true;
        res := res or $10000000;
          //дополнительный признак для принудительного вывода строки
      end;
...
begin
  ...
  begin
    ...
    DateTimeStr := SetTimeStamp(ListsCmd2[NumListCmd].isVisTime,
      TerminalST.Form1.isAddDate, CurTimeRB);
    ...
    MemoUpd(CurTimeRB, BeginStr, RxDataLog, RxDataLogASCII, RxDataLogHEX,
      RxDataLogDec, RxDataLogCUSTOM, true, FontColorRead);
    ...
    if isUseParserHEX then
    begin
      FormParser.SetNewStr(SrcPortMain, RxDataLogHEX, ReadMode = ReadHEX,
        CurTimeRB);
    end;

    if isUseParserASCII then
      FormParserASCII.SetNewStr1(SrcPortMain, RxDataLog, ReadMode = ReadHEX,
        CurTimeRB, RxBytes);
    ...
    begin
      ...
      begin
        ...
        begin
          SendPortData(OldCmdList.Items[indexCmdInFile], AddStrCmd.Text,
            SendMode, true);
          inc(indexCmdInFile);
          if indexCmdInFile >= OldCmdList.Count then
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          FormEvent.ShowModal;
          isOpenMsgEventBytesRecDataEn[NumListCmd] := not
            FormEvent.isNotShowEvent;
        end;
      end;
    end
    else
    ...
    if ((Cnt > 0) and (ReadMode = ReadASCII)) then
      isOut := (((RxBufData[Cnt - 1] = $0A) and isSep0A) or
        ...
        ((RxBufData[Cnt - 2] = $0D) and (RxBufData[Cnt - 1] = $0A) and (Cnt > 1)
          and isSep0D0A));

    isOut := (isOut) or (isSepAftSeq1_4 and (Cnt > 3) and (SepAftByte1_4_1 =
      RxBufData[Cnt - 4])
      and (SepAftByte1_4_2 = RxBufData[Cnt - 3])
      and (SepAftByte1_4_3 = RxBufData[Cnt - 2])
      and (SepAftByte1_4_4 = RxBufData[Cnt - 1]));

    isOut := (isOut) or (isSepAftSeq1_3 and (Cnt > 2) and (SepAftByte1_3_1 =
      RxBufData[Cnt - 3])
      and (SepAftByte1_3_2 = RxBufData[Cnt - 2])
      and (SepAftByte1_3_3 = RxBufData[Cnt - 1]));

    isOut := (isOut) or (isSepAftSeq1_2 and (Cnt > 1) and (SepAftByte1_2_1 =
      RxBufData[Cnt - 2])
      and (SepAftByte1_2_2 = RxBufData[Cnt - 1]));

    isOut := (isOut) or ((Cnt > 0) and (isSepSymbol) and (RxBufData[Cnt - 1] =
      SepSymbol));

    ////
    isOut := (isOut) or (isSepAftSeq2_4 and (Cnt > 3) and (SepAftByte2_4_1 =
      RxBufData[Cnt - 4])
      and (SepAftByte2_4_2 = RxBufData[Cnt - 3])
      and (SepAftByte2_4_3 = RxBufData[Cnt - 2])
      and (SepAftByte2_4_4 = RxBufData[Cnt - 1]));

    isOut := (isOut) or (isSepAftSeq2_3 and (Cnt > 2) and (SepAftByte2_3_1 =
      RxBufData[Cnt - 3])
      and (SepAftByte2_3_2 = RxBufData[Cnt - 2])
      and (SepAftByte2_3_3 = RxBufData[Cnt - 1]));

    isOut := (isOut) or (isSepAftSeq2_2 and (Cnt > 1) and (SepAftByte2_2_1 =
      RxBufData[Cnt - 2])
      and (SepAftByte2_2_2 = RxBufData[Cnt - 1]));

    isOut := (isOut) or ((Cnt > 0) and (isSepSymbol2) and (RxBufData[Cnt - 1] =
      SepSymbol2));
    ...
    begin
      ...
      begin
        WriteLogEvent(Now, 'No read data ' + IntToStr(TimeAutoExpanNoData) +
          ' ms');
        TimeLastRead := MSec;
        if (isExpWin) and (not IsIconic(Form1.Handle)) then
        begin
        ...
        begin
          FormEvent.SrcEvent := SrcEventMain;
          FormEvent.EventMSG := 'No read data ' + IntToStr(TimeAutoExpanNoData) +
            ' ms';
          FormEvent.ShowModal;
          isOpenMsgEventNoIncomingDataEn[NumListCmd] := not
            FormEvent.isNotShowEvent;
        end;
      end;
    end
    ...
    isSendMemo := (isOut) or ((MSec - MSecRefRX > TimeOutAnsRX) and isSepTime
      and (GlBufRx.Shift > 0));
...
begin
  ...
  begin
    if (indexCmdInFile < OldCmdList.Count) then
    begin
      SendPortData(OldCmdList.Items[indexCmdInFile], AddStrCmd.Text, SendMode,
        true);
      inc(indexCmdInFile);
      if (indexCmdInFile = OldCmdList.Count) then
      begin
...
begin
  ...
  begin
    SetEnHideTimer.Enabled := true;
    isEnHide := false;
    MainStrCmd.Text := (Sender as TListBox).Items[(Sender as
      TListBox).ItemIndex];
...
begin
  ...
  StaticText3.Caption := 'T:' + IntToStr(CntTX) + ' R:' + IntToStr(CntRXRead) +
    ' O:' + IntToStr(CntRXOut) + ' OF:' + TmpStr + '(' + TmpStr2 + ')%';
  ...
  begin
    ...
    begin
      ...
      if (CoordMouseRefHelpConnect.X = CoordMouseCur.X) and
        (CoordMouseRefHelpConnect.Y = CoordMouseCur.Y) then
        OutHelpConnect;
    end;
  end;
  /////////////////////////////////////
  ...
  begin
    ...
    begin
      GetCursorPos(CoordMouseCur);
      if (CoordMouseRefHelpRescan.X = CoordMouseCur.X) and
        (CoordMouseRefHelpRescan.Y = CoordMouseCur.Y) then
        OutHelpRescan;
    end;
  end;
  /////////////////////////////////////
  ...
  begin
    ...
    begin
      GetCursorPos(CoordMouseCur);
      if (CoordMouseRefHelpAutoConnect.X = CoordMouseCur.X) and
        (CoordMouseRefHelpAutoConnect.Y = CoordMouseCur.Y) then
        OutHelpAutoConnect;
    end;
  end;
  /////////////////////////////////////
  ...
  begin
    ...
    begin
      GetCursorPos(CoordMouseCur);
      if (CoordMouseRefHelpTimeRX.X = CoordMouseCur.X) and
        (CoordMouseRefHelpTimeRX.Y = CoordMouseCur.Y) then
        OutHelpTimeRX;
    end;
  end;
  /////////////////////////////////////
  ...
  begin
    ...
    begin
      GetCursorPos(CoordMouseCur);
      if (CoordMouseRefHelpFiltr.X = CoordMouseCur.X) and
        (CoordMouseRefHelpFiltr.Y = CoordMouseCur.Y) then
        OutHelpFiltr;
    end;
  end;
  /////////////////////////////////////
  ...
  begin
    ...
    begin
      GetCursorPos(CoordMouseCur);
      if (CoordMouseRefHelpModeManual.X = CoordMouseCur.X) and
        (CoordMouseRefHelpModeManual.Y = CoordMouseCur.Y) then
        OutHelpModeManual;
    end;
  end;
  /////////////////////////////////////
  ...
  begin
    ...
    begin
      GetCursorPos(CoordMouseCur);
      if (CoordMouseRefHelpModeSel.X = CoordMouseCur.X) and
        (CoordMouseRefHelpModeSel.Y = CoordMouseCur.Y) then
        OutHelpModeSel;
    end;
  end;
  /////////////////////////////////////
  ...
  begin
    ...
    begin
      GetCursorPos(CoordMouseCur);
      if (CoordMouseRefHelpModeAfterAns.X = CoordMouseCur.X) and
        (CoordMouseRefHelpModeAfterAns.Y = CoordMouseCur.Y) then
        OutHelpModeAfterAns;
    end;
  end;
  /////////////////////////////////////
  ...
  begin
    ...
    begin
      GetCursorPos(CoordMouseCur);
      if (CoordMouseRefHelpModePeriod.X = CoordMouseCur.X) and
        (CoordMouseRefHelpModePeriod.Y = CoordMouseCur.Y) then
        OutHelpModePeriod;
    end;
  end;
  /////////////////////////////////////
  ...
  begin
    ...
    begin
      GetCursorPos(CoordMouseCur);
      if (CoordMouseRefHelpForConnect.X = CoordMouseCur.X) and
        (CoordMouseRefHelpForConnect.Y = CoordMouseCur.Y) then
        OutHelpForConnect;
    end;
  end;
  /////////////////////////////////////
  ...
  begin
    ...
    begin
      GetCursorPos(CoordMouseCur);
      if (CoordMouseRefHelpConsol.X = CoordMouseCur.X) and
        (CoordMouseRefHelpConsol.Y = CoordMouseCur.Y) then
        OutHelpConsol;
    end;
  end;
  /////////////////////////////////////
  ...
  begin
    ...
    begin
      GetCursorPos(CoordMouseCur);
      if (CoordMouseRefHelpAddConsol.X = CoordMouseCur.X) and
        (CoordMouseRefHelpAddConsol.Y = CoordMouseCur.Y) then
        OutHelpAddConsol;
    end;
  end;
  /////////////////////////////////////
  ...
  begin
    ...
    begin
      GetCursorPos(CoordMouseCur);
      if (CoordMouseRefHelpCiklSend.X = CoordMouseCur.X) and
        (CoordMouseRefHelpCiklSend.Y = CoordMouseCur.Y) then
        OutHelpCiklSend;
    end;
  end;
  /////////////////////////////////////
...
begin
  ...
  Form1.STH2.Caption :=
    ' A description of the commands will be displayed here.';
  Form1.STH3.Caption := ' Editable help is displayed when you hover the mouse';
  Form1.STH4.Caption :=
    ' pointer over the button with  the corresponding command';
  Form1.STH5.Caption := '                 ';
  ...
  Form1.STH10.Caption :=
    ' in a stationary state over the corresponding control.';
  Form1.STH11.Caption := ' ';
  Form1.STH12.Caption := ' ';
end;
...
begin
  ...
  begin
    ...
    AddStrCmd.Text := (Sender as TListBox).Items[(Sender as
      TListBox).ItemIndex];
    (Sender as TListBox).Visible := false;
    if isModeDialog then
...
begin
  ...
  begin
    ...
    begin
      ...
      SendFileLavel.Caption := 'SendFile: ' + IntToStr(IndexInSendFile - 1) + ' '
        + IntToStr(MAXSendDataFile);
      isSendFile := True;
...
begin
  ...
  begin
    ShellExecute(Handle, 'open', 'notepad.exe',
      PChar(OpenLogFileDialog.FileName), nil, SW_SHOWNORMAL);
  end;
end;
...
begin
  ...
  begin
    ShellExecute(Handle, 'open', 'notepad.exe',
      PChar(OpenBinFileDialog.FileName), nil, SW_SHOWNORMAL);
  end;
end;
...
begin
  if ((Sender as TEdit).Text <> '') and (Length((Sender as TEdit).Text) < 8)
    then
  begin
...
begin
  if ((Sender as TEdit).Text <> '') and (Length((Sender as TEdit).Text) < 5)
    then
  begin
...
begin
  ...
  if ((Sender as TEdit).Text <> '') and (Length((Sender as TEdit).Text) = 2)
    then
  begin
...
begin
  if ((Sender as TEdit).Text <> '') and (Length((Sender as TEdit).Text) < 10)
    then
  begin
    (Sender as TEdit).Color := clWindow;
...
begin
  ...
  BtPasteList.Hint := 'Paste list "' + CopyListCmdBuf.NameList +
    '" from clipboard';
...
begin
  ...
  ListsCmd2[NumListCmd].SeqSymbols1_3 := ListsCmd2[NumListCmd].SeqSymbols1_3 and
    $00FFFFFF;
  ListsCmd2[NumListCmd].SeqSymbols1_3 := ListsCmd2[NumListCmd].SeqSymbols1_3 or
    (SepAftByte1_3_1 shl 24);
...
begin
  ...
  ListsCmd2[NumListCmd].SeqSymbols2_4 := ListsCmd2[NumListCmd].SeqSymbols2_4 and
    $FF00FFFF;
  ListsCmd2[NumListCmd].SeqSymbols2_4 := ListsCmd2[NumListCmd].SeqSymbols2_4 or
    (SepAftByte2_4_2 shl 16);
...
begin
  ...
  ListsCmd2[NumListCmd].SeqSymbols2_3 := ListsCmd2[NumListCmd].SeqSymbols2_3 and
    $00FFFFFF;
  ListsCmd2[NumListCmd].SeqSymbols2_3 := ListsCmd2[NumListCmd].SeqSymbols2_3 or
    (SepAftByte2_3_1 shl 24);
...
begin
  ...
  ListsCmd2[NumListCmd].SeqSymbols2_2 := ListsCmd2[NumListCmd].SeqSymbols2_2 and
    $00FFFFFF;
  ListsCmd2[NumListCmd].SeqSymbols2_2 := ListsCmd2[NumListCmd].SeqSymbols2_2 or
    (SepAftByte2_2_1 shl 24);
...
begin
  ...
  if ListsCmd2[NumListCmd].isUseHotKey = False then
    ReadListCmd(NumListCmd);
  //(Sender as TCheckBox).ShowHint := False;
  if isModeDialog and (ActiveControl <> ListDec) and (ActiveControl <> ListInc)
    then
    ActiveControl := BtSend;
end;
...
begin
  ...
  case num of
    1: ListsCmd2[NumListCmd].HK1_4 := ((ListsCmd2[NumListCmd].HK1_4 and
      $FFFFFF00) or (key shl 0));
    2: ListsCmd2[NumListCmd].HK1_4 := ((ListsCmd2[NumListCmd].HK1_4 and
      $FFFF00FF) or (key shl 8));
    3: ListsCmd2[NumListCmd].HK1_4 := ((ListsCmd2[NumListCmd].HK1_4 and
      $FF00FFFF) or (key shl 16));
    4: ListsCmd2[NumListCmd].HK1_4 := ((ListsCmd2[NumListCmd].HK1_4 and
      $00FFFFFF) or (key shl 24));

    5: ListsCmd2[NumListCmd].HK5_8 := ((ListsCmd2[NumListCmd].HK5_8 and
      $FFFFFF00) or (key shl 0));
    6: ListsCmd2[NumListCmd].HK5_8 := ((ListsCmd2[NumListCmd].HK5_8 and
      $FFFF00FF) or (key shl 8));
    7: ListsCmd2[NumListCmd].HK5_8 := ((ListsCmd2[NumListCmd].HK5_8 and
      $FF00FFFF) or (key shl 16));
    8: ListsCmd2[NumListCmd].HK5_8 := ((ListsCmd2[NumListCmd].HK5_8 and
      $00FFFFFF) or (key shl 24));

    9: ListsCmd2[NumListCmd].HK9_12 := ((ListsCmd2[NumListCmd].HK9_12 and
      $FFFFFF00) or (key shl 0));
    10: ListsCmd2[NumListCmd].HK9_12 := ((ListsCmd2[NumListCmd].HK9_12 and
      $FFFF00FF) or (key shl 8));
    11: ListsCmd2[NumListCmd].HK9_12 := ((ListsCmd2[NumListCmd].HK9_12 and
      $FF00FFFF) or (key shl 16));
    12: ListsCmd2[NumListCmd].HK9_12 := ((ListsCmd2[NumListCmd].HK9_12 and
      $00FFFFFF) or (key shl 24));

    13: ListsCmd2[NumListCmd].HK13_16 := ((ListsCmd2[NumListCmd].HK13_16 and
      $FFFFFF00) or (key shl 0));
    14: ListsCmd2[NumListCmd].HK13_16 := ((ListsCmd2[NumListCmd].HK13_16 and
      $FFFF00FF) or (key shl 8));
    15: ListsCmd2[NumListCmd].HK13_16 := ((ListsCmd2[NumListCmd].HK13_16 and
      $FF00FFFF) or (key shl 16));
    16: ListsCmd2[NumListCmd].HK13_16 := ((ListsCmd2[NumListCmd].HK13_16 and
      $00FFFFFF) or (key shl 24));

    17: ListsCmd2[NumListCmd].HK17_20 := ((ListsCmd2[NumListCmd].HK17_20 and
      $FFFFFF00) or (key shl 0));
    18: ListsCmd2[NumListCmd].HK17_20 := ((ListsCmd2[NumListCmd].HK17_20 and
      $FFFF00FF) or (key shl 8));
    19: ListsCmd2[NumListCmd].HK17_20 := ((ListsCmd2[NumListCmd].HK17_20 and
      $FF00FFFF) or (key shl 16));
    20: ListsCmd2[NumListCmd].HK17_20 := ((ListsCmd2[NumListCmd].HK17_20 and
      $00FFFFFF) or (key shl 24));

    21: ListsCmd2[NumListCmd].HK21_24 := ((ListsCmd2[NumListCmd].HK21_24 and
      $FFFFFF00) or (key shl 0));
    22: ListsCmd2[NumListCmd].HK21_24 := ((ListsCmd2[NumListCmd].HK21_24 and
      $FFFF00FF) or (key shl 8));
    23: ListsCmd2[NumListCmd].HK21_24 := ((ListsCmd2[NumListCmd].HK21_24 and
      $FF00FFFF) or (key shl 16));
    24: ListsCmd2[NumListCmd].HK21_24 := ((ListsCmd2[NumListCmd].HK21_24 and
      $00FFFFFF) or (key shl 24));

    25: ListsCmd2[NumListCmd].HK25_28 := ((ListsCmd2[NumListCmd].HK25_28 and
      $FFFFFF00) or (key shl 0));
    26: ListsCmd2[NumListCmd].HK25_28 := ((ListsCmd2[NumListCmd].HK25_28 and
      $FFFF00FF) or (key shl 8));
    27: ListsCmd2[NumListCmd].HK25_28 := ((ListsCmd2[NumListCmd].HK25_28 and
      $FF00FFFF) or (key shl 16));
    28: ListsCmd2[NumListCmd].HK25_28 := ((ListsCmd2[NumListCmd].HK25_28 and
      $00FFFFFF) or (key shl 24));

    29: ListsCmd2[NumListCmd].HK29_32 := ((ListsCmd2[NumListCmd].HK29_32 and
      $FFFFFF00) or (key shl 0));
    30: ListsCmd2[NumListCmd].HK29_32 := ((ListsCmd2[NumListCmd].HK29_32 and
      $FFFF00FF) or (key shl 8));
    31: ListsCmd2[NumListCmd].HK29_32 := ((ListsCmd2[NumListCmd].HK29_32 and
      $FF00FFFF) or (key shl 16));
    32: ListsCmd2[NumListCmd].HK29_32 := ((ListsCmd2[NumListCmd].HK29_32 and
      $00FFFFFF) or (key shl 24));

    33: ListsCmd2[NumListCmd].HK33_36 := ((ListsCmd2[NumListCmd].HK33_36 and
      $FFFFFF00) or (key shl 0));
    34: ListsCmd2[NumListCmd].HK33_36 := ((ListsCmd2[NumListCmd].HK33_36 and
      $FFFF00FF) or (key shl 8));
    35: ListsCmd2[NumListCmd].HK33_36 := ((ListsCmd2[NumListCmd].HK33_36 and
      $FF00FFFF) or (key shl 16));
    36: ListsCmd2[NumListCmd].HK33_36 := ((ListsCmd2[NumListCmd].HK33_36 and
      $00FFFFFF) or (key shl 24));

    37: ListsCmd2[NumListCmd].HK37_40 := ((ListsCmd2[NumListCmd].HK37_40 and
      $FFFFFF00) or (key shl 0));
    38: ListsCmd2[NumListCmd].HK37_40 := ((ListsCmd2[NumListCmd].HK37_40 and
      $FFFF00FF) or (key shl 8));
    39: ListsCmd2[NumListCmd].HK37_40 := ((ListsCmd2[NumListCmd].HK37_40 and
      $FF00FFFF) or (key shl 16));
    40: ListsCmd2[NumListCmd].HK37_40 := ((ListsCmd2[NumListCmd].HK37_40 and
      $00FFFFFF) or (key shl 24));

    41: ListsCmd2[NumListCmd].HK41_44 := ((ListsCmd2[NumListCmd].HK41_44 and
      $FFFFFF00) or (key shl 0));
    42: ListsCmd2[NumListCmd].HK41_44 := ((ListsCmd2[NumListCmd].HK41_44 and
      $FFFF00FF) or (key shl 8));
    43: ListsCmd2[NumListCmd].HK41_44 := ((ListsCmd2[NumListCmd].HK41_44 and
      $FF00FFFF) or (key shl 16));
    44: ListsCmd2[NumListCmd].HK41_44 := ((ListsCmd2[NumListCmd].HK41_44 and
      $00FFFFFF) or (key shl 24));

    45: ListsCmd2[NumListCmd].HK45_48 := ((ListsCmd2[NumListCmd].HK45_48 and
      $FFFFFF00) or (key shl 0));
    46: ListsCmd2[NumListCmd].HK45_48 := ((ListsCmd2[NumListCmd].HK45_48 and
      $FFFF00FF) or (key shl 8));
    47: ListsCmd2[NumListCmd].HK45_48 := ((ListsCmd2[NumListCmd].HK45_48 and
      $FF00FFFF) or (key shl 16));
    48: ListsCmd2[NumListCmd].HK45_48 := ((ListsCmd2[NumListCmd].HK45_48 and
      $00FFFFFF) or (key shl 24));
  end;
...
begin
  ...
  case i of
    1: ListsCmd2[NumListCmd].J01_02 := (ListsCmd2[NumListCmd].J01_02 and
      $FFFF0000) or (tmp shl 0);
    2: ListsCmd2[NumListCmd].J01_02 := (ListsCmd2[NumListCmd].J01_02 and
      $0000FFFF) or (tmp shl 16);
    3: ListsCmd2[NumListCmd].J03_04 := (ListsCmd2[NumListCmd].J03_04 and
      $FFFF0000) or (tmp shl 0);
    4: ListsCmd2[NumListCmd].J03_04 := (ListsCmd2[NumListCmd].J03_04 and
      $0000FFFF) or (tmp shl 16);
    5: ListsCmd2[NumListCmd].J05_06 := (ListsCmd2[NumListCmd].J05_06 and
      $FFFF0000) or (tmp shl 0);
    6: ListsCmd2[NumListCmd].J05_06 := (ListsCmd2[NumListCmd].J05_06 and
      $0000FFFF) or (tmp shl 16);
    7: ListsCmd2[NumListCmd].J07_08 := (ListsCmd2[NumListCmd].J07_08 and
      $FFFF0000) or (tmp shl 0);
    8: ListsCmd2[NumListCmd].J07_08 := (ListsCmd2[NumListCmd].J07_08 and
      $0000FFFF) or (tmp shl 16);
    9: ListsCmd2[NumListCmd].J09_10 := (ListsCmd2[NumListCmd].J09_10 and
      $FFFF0000) or (tmp shl 0);
    10: ListsCmd2[NumListCmd].J09_10 := (ListsCmd2[NumListCmd].J09_10 and
      $0000FFFF) or (tmp shl 16);
    11: ListsCmd2[NumListCmd].J11_12 := (ListsCmd2[NumListCmd].J11_12 and
      $FFFF0000) or (tmp shl 0);
    12: ListsCmd2[NumListCmd].J11_12 := (ListsCmd2[NumListCmd].J11_12 and
      $0000FFFF) or (tmp shl 16);
    13: ListsCmd2[NumListCmd].J13_14 := (ListsCmd2[NumListCmd].J13_14 and
      $FFFF0000) or (tmp shl 0);
    14: ListsCmd2[NumListCmd].J13_14 := (ListsCmd2[NumListCmd].J13_14 and
      $0000FFFF) or (tmp shl 16);
    15: ListsCmd2[NumListCmd].J15_16 := (ListsCmd2[NumListCmd].J15_16 and
      $FFFF0000) or (tmp shl 0);
    16: ListsCmd2[NumListCmd].J15_16 := (ListsCmd2[NumListCmd].J15_16 and
      $0000FFFF) or (tmp shl 16);
    17: ListsCmd2[NumListCmd].J17_18 := (ListsCmd2[NumListCmd].J17_18 and
      $FFFF0000) or (tmp shl 0);
    18: ListsCmd2[NumListCmd].J17_18 := (ListsCmd2[NumListCmd].J17_18 and
      $0000FFFF) or (tmp shl 16);
    19: ListsCmd2[NumListCmd].J19_20 := (ListsCmd2[NumListCmd].J19_20 and
      $FFFF0000) or (tmp shl 0);
    20: ListsCmd2[NumListCmd].J19_20 := (ListsCmd2[NumListCmd].J19_20 and
      $0000FFFF) or (tmp shl 16);
    21: ListsCmd2[NumListCmd].J21_22 := (ListsCmd2[NumListCmd].J21_22 and
      $FFFF0000) or (tmp shl 0);
    22: ListsCmd2[NumListCmd].J21_22 := (ListsCmd2[NumListCmd].J21_22 and
      $0000FFFF) or (tmp shl 16);
    23: ListsCmd2[NumListCmd].J23_24 := (ListsCmd2[NumListCmd].J23_24 and
      $FFFF0000) or (tmp shl 0);
    24: ListsCmd2[NumListCmd].J23_24 := (ListsCmd2[NumListCmd].J23_24 and
      $0000FFFF) or (tmp shl 16);
    25: ListsCmd2[NumListCmd].J25_26 := (ListsCmd2[NumListCmd].J25_26 and
      $FFFF0000) or (tmp shl 0);
    26: ListsCmd2[NumListCmd].J25_26 := (ListsCmd2[NumListCmd].J25_26 and
      $0000FFFF) or (tmp shl 16);
    27: ListsCmd2[NumListCmd].J27_28 := (ListsCmd2[NumListCmd].J27_28 and
      $FFFF0000) or (tmp shl 0);
    28: ListsCmd2[NumListCmd].J27_28 := (ListsCmd2[NumListCmd].J27_28 and
      $0000FFFF) or (tmp shl 16);
    29: ListsCmd2[NumListCmd].J29_30 := (ListsCmd2[NumListCmd].J29_30 and
      $FFFF0000) or (tmp shl 0);
    30: ListsCmd2[NumListCmd].J29_30 := (ListsCmd2[NumListCmd].J29_30 and
      $0000FFFF) or (tmp shl 16);
    31: ListsCmd2[NumListCmd].J31_32 := (ListsCmd2[NumListCmd].J31_32 and
      $FFFF0000) or (tmp shl 0);
    32: ListsCmd2[NumListCmd].J31_32 := (ListsCmd2[NumListCmd].J31_32 and
      $0000FFFF) or (tmp shl 16);
    33: ListsCmd2[NumListCmd].J33_34 := (ListsCmd2[NumListCmd].J33_34 and
      $FFFF0000) or (tmp shl 0);
    34: ListsCmd2[NumListCmd].J33_34 := (ListsCmd2[NumListCmd].J33_34 and
      $0000FFFF) or (tmp shl 16);
    35: ListsCmd2[NumListCmd].J35_36 := (ListsCmd2[NumListCmd].J35_36 and
      $FFFF0000) or (tmp shl 0);
    36: ListsCmd2[NumListCmd].J35_36 := (ListsCmd2[NumListCmd].J35_36 and
      $0000FFFF) or (tmp shl 16);
    37: ListsCmd2[NumListCmd].J37_38 := (ListsCmd2[NumListCmd].J37_38 and
      $FFFF0000) or (tmp shl 0);
    38: ListsCmd2[NumListCmd].J37_38 := (ListsCmd2[NumListCmd].J37_38 and
      $0000FFFF) or (tmp shl 16);
    39: ListsCmd2[NumListCmd].J39_40 := (ListsCmd2[NumListCmd].J39_40 and
      $FFFF0000) or (tmp shl 0);
    40: ListsCmd2[NumListCmd].J39_40 := (ListsCmd2[NumListCmd].J39_40 and
      $0000FFFF) or (tmp shl 16);
    41: ListsCmd2[NumListCmd].J41_42 := (ListsCmd2[NumListCmd].J41_42 and
      $FFFF0000) or (tmp shl 0);
    42: ListsCmd2[NumListCmd].J41_42 := (ListsCmd2[NumListCmd].J41_42 and
      $0000FFFF) or (tmp shl 16);
    43: ListsCmd2[NumListCmd].J43_44 := (ListsCmd2[NumListCmd].J43_44 and
      $FFFF0000) or (tmp shl 0);
    44: ListsCmd2[NumListCmd].J43_44 := (ListsCmd2[NumListCmd].J43_44 and
      $0000FFFF) or (tmp shl 16);
    45: ListsCmd2[NumListCmd].J45_46 := (ListsCmd2[NumListCmd].J45_46 and
      $FFFF0000) or (tmp shl 0);
    46: ListsCmd2[NumListCmd].J45_46 := (ListsCmd2[NumListCmd].J45_46 and
      $0000FFFF) or (tmp shl 16);
    47: ListsCmd2[NumListCmd].J47_48 := (ListsCmd2[NumListCmd].J47_48 and
      $FFFF0000) or (tmp shl 0);
    48: ListsCmd2[NumListCmd].J47_48 := (ListsCmd2[NumListCmd].J47_48 and
      $0000FFFF) or (tmp shl 16);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if kt > 0 then
        begin
          if ((SettJ[i].numJ = 1) and ((kt and k1) = kt)) or ((SettJ[i].numJ = 2)
            and ((kt and k2) = kt)) then
          begin
            if LastKeyJ = 0 then
            begin
              BTCmd[i].Click();
...
begin
  ...
  for num := 1 to 48 do
  begin
    if (SettJ[numCur].numJ = SettJ[num].numJ) and (SettJ[numCur].Key1 =
      SettJ[num].Key1) and
      (SettJ[numCur].Key2 = SettJ[num].Key2) and (SettJ[numCur].isUse) and
        (SettJ[num].isUse) then
    begin
...
begin
  ...
  ListsCmd2[NumListCmd].SeqSymbols1_4 := ListsCmd2[NumListCmd].SeqSymbols1_4 and
    $00FFFFFF;
  ListsCmd2[NumListCmd].SeqSymbols1_4 := ListsCmd2[NumListCmd].SeqSymbols1_4 or
    (SepAftByte1_4_1 shl 24);
...
begin
  ...
  ListsCmd2[NumListCmd].SeqSymbols1_2 := ListsCmd2[NumListCmd].SeqSymbols1_2 and
    $00FFFFFF;
  ListsCmd2[NumListCmd].SeqSymbols1_2 := ListsCmd2[NumListCmd].SeqSymbols1_2 or
    (SepAftByte1_2_1 shl 24);
...
begin
  SendAddPortData(MainStrCmd.Text, AddStrCmd.Text, SendMode, (Sender as
    TButton).Tag);
end;

procedure TForm1.SetActBtSendInAdd(nPort: Integer; isActiv: Boolean; Name:
  string; Hint: string);
begin
  case nPort of
    1:
    ...
    7:
      begin
        Bt10.Enabled := isActiv;
        Bt10.Caption := Name;
        Bt10.Hint := Hint;
      end;
    8:
      begin
        Bt11.Enabled := isActiv;
        Bt11.Caption := Name;
        Bt11.Hint := Hint;
      end;
    9:
      begin
        Bt12.Enabled := isActiv;
        Bt12.Caption := Name;
        Bt12.Hint := Hint;
      end;
    10:
      begin
        Bt13.Enabled := isActiv;
        Bt13.Caption := Name;
        Bt13.Hint := Hint;
      end;
    11:
      begin
        Bt14.Enabled := isActiv;
        Bt14.Caption := Name;
        Bt14.Hint := Hint;
      end;
    12:
      begin
        Bt15.Enabled := isActiv;
        Bt15.Caption := Name;
        Bt15.Hint := Hint;
      end;
    13:
      begin
        Bt16.Enabled := isActiv;
        Bt16.Caption := Name;
        Bt16.Hint := Hint;
      end;
    14:
      begin
        Bt17.Enabled := isActiv;
        Bt17.Caption := Name;
        Bt17.Hint := Hint;
      end;
    15:
      begin
        Bt18.Enabled := isActiv;
        Bt18.Caption := Name;
        Bt18.Hint := Hint;
      end;
    16:
      begin
        Bt19.Enabled := isActiv;
        Bt19.Caption := Name;
        Bt19.Hint := Hint;
      end;
    17:
      begin
        Bt20.Enabled := isActiv;
        Bt20.Caption := Name;
        Bt20.Hint := Hint;
      end;
    18:
      begin
        Bt21.Enabled := isActiv;
        Bt21.Caption := Name;
        Bt21.Hint := Hint;
      end;
  end;
...
begin
  ...
  if Bt9.Enabled then
    Bt9.Click;
  if Bt10.Enabled then
    Bt10.Click;
  if Bt11.Enabled then
    Bt11.Click;
  if Bt12.Enabled then
    Bt12.Click;
  if Bt13.Enabled then
    Bt13.Click;
  if Bt14.Enabled then
    Bt14.Click;
  if Bt15.Enabled then
    Bt15.Click;
  if Bt16.Enabled then
    Bt16.Click;
  if Bt17.Enabled then
    Bt17.Click;
  if Bt18.Enabled then
    Bt18.Click;
  if Bt19.Enabled then
    Bt19.Click;
  if Bt20.Enabled then
    Bt20.Click;
  if Bt21.Enabled then
    Bt21.Click;
...
begin
  ...
  if (Form1.Left + Form1.Width > Screen.Width) and (Screen.MonitorCount = 1)
    then
    Form1.Left := Screen.Width - Form1.Width;

  if (Form1.Left + Form1.Width > Screen.Width) and (Screen.MonitorCount = 2)
    then
    Form1.Left := Screen.Monitors[0].Width + Screen.Monitors[1].Width -
      Form1.Width;

  if (Form1.Left + Form1.Width > Screen.Width) and (Screen.MonitorCount = 3)
    then
    Form1.Left := Screen.Monitors[0].Width + Screen.Monitors[1].Width +
      Screen.Monitors[2].Width - Form1.Width;

  if (Form1.Left + Form1.Width > Screen.Width) and (Screen.MonitorCount = 4)
    then
    Form1.Left := Screen.Monitors[0].Width + Screen.Monitors[1].Width +
      Screen.Monitors[2].Width + Screen.Monitors[3].Width - Form1.Width;
...
begin
  ...
  begin
    try
      ...
      if FileExists(AutoLogFileBin) then
        Seek(LogBinFile, FileSize(LogBinFile))
      else
        ReWrite(LogBinFile);
  ...
  begin
    try
      ...
      if FileExists(AutoLogFileTxt) then
        Append(LogFile)
      else
        ReWrite(LogFile);
